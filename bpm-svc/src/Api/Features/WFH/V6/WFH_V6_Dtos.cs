...
public sealed record WFH_V6_DateRange(DateOnly Start, DateOnly End);

public sealed record WFH_V6_SubmitRequest(
    DateOnly ApplyDate,
    WFH_V6_DateRange DateRange,
    string Reason,
    Guid? AttachmentFileId);

public sealed record WFH_V6_DecisionRequest(bool Approve, string? Comment);

public sealed record WFH_V6_DecisionDto(
    Guid? UserId, string? DisplayName, bool? Approved, string? Comment, DateTime? DecidedAt);

public sealed record WFH_V6_CaseResponse(
    Guid Id,
    ...
    WFH_V6_DecisionDto? ManagerDecision,
    WFH_V6_DecisionDto? SeniorDecision,
    DateTime SubmittedAt,
    DateTime LastActivityAt,
    DateTime? CompletedAt);

public sealed record WFH_V6_CaseRowResponse(
    Guid Id,
    int Days,
    DateOnly StartDate,
    DateOnly EndDate,
...
internal static class WFH_V6_DtoMapping
{
    public static WFH_V6_CaseResponse ToResponse(WFH_V6_Case c, IReadOnlyDictionary<Guid, string> names)
        => new(
            Id: c.Id,
            ...
            ManagerDecision: c.ManagerUserId is null ? null : new WFH_V6_DecisionDto(
                c.ManagerUserId, names.GetValueOrDefault(c.ManagerUserId.Value),
                c.ManagerApproved, c.ManagerComment, c.ManagerDecisionAt),
            SeniorDecision: c.SeniorUserId is null ? null : new WFH_V6_DecisionDto(
    ...
    public static WFH_V6_WfhService.SubmitInput ToServiceInput(Guid submitterUserId, WFH_V6_SubmitRequest req)
