import type { WFH_V6_Status, WFH_V6_SubmitPayload } from './WFH_V6_types'

export function emptyPayload(): WFH_V6_SubmitPayload {
  return { applyDate: '', start: '', end: '', reason: '', attachmentFileId: null }
}

/**
 * Senior approval (上級主管) kicks in at this many consecutive days
 * **or more** (spec gateway_days: `days >= 100`). V6 raised V5's >= 90 gate.
 */
export const DAYS_GATE_THRESHOLD = 100

/** Whether a request needs the senior (上級主管) stage — mirrors the C# gate `c.Days >= 100`. */
 ...
 * `WFH_V6_WfhService.ComputeConsecutiveDays`; the server recomputes and
...
export function zhStatus(s: WFH_V6_Status): string {
  switch (s) {
