...
import { getSandboxStatus, getSandboxClock, getSandboxUnreadCount } from '@/lib/api/sandbox'
...
/**
 * PR-J5 §10.4: end-user banner. Mirrors the admin banner so testers see the
 * same captured / clock-offset numbers across both UIs.
 */
export function SandboxBanner() {
  const [state, setState] = useState<State>({ enabled: false, unread: null, clock: null })
    ...
    async function tick() {
      try {
        ...
        const [unread, clock] = await Promise.all([
          getSandboxUnreadCount().catch(() => null),
          getSandboxClock().catch(() => null),
        ])
        if (cancelled) return
