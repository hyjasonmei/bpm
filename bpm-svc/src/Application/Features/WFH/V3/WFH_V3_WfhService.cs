...
/// <summary>
/// State machine for WFH (居家辦公申請) V3. One transition per public
/// method. V3 raises the senior-approval gateway threshold from V2's
/// ≥ 15 days to <b>≥ 30 days</b> (bundle edges e4 <c>days &lt; 30</c> →
/// end_approved, e5 <c>days &gt;= 30</c> → approval_senior). The graph is
/// otherwise identical to V1/V2. Status graph (bundle nodes start_1 /
/// task_apply / approval_manager / gateway_days / approval_senior /
/// end_*):
/// <code>
///   start ─► PendingManager ──approve, days &lt; 30──► Completed
///                  │                  │
///                  │              approve, days &gt;= 30
///                  │                  ▼
...
/// </summary>
public sealed class WFH_V3_WfhService(
    IWFH_V3_CaseStore store,
    IOrgChartReader org,
    IPrincipalDirectory directory,
    IClock clock,
    ILogger<WFH_V3_WfhService> log,
    INotifyDispatcher notify,
    IActorAuthorizer auth)
{
    public const string FlowCode = "WFH";
    public const int FlowVersion = 3;

    /// <summary>gateway_days threshold — consecutive days &gt;= 30 routes to senior approval.</summary>
    public const int DaysGateThreshold = 30;
    ...
    public async Task<WFH_V3_Case> SubmitAsync(SubmitInput input, CancellationToken ct)
    {
        ...
        var c = new WFH_V3_Case
        {
            ...
            Status = WFH_V3_CaseStatus.PendingManager,
        ...
        Guid caseId, Guid actorUserId, SubmitInput input, CancellationToken ct)
    {
        ...
        c.Status = WFH_V3_CaseStatus.PendingManager;
        c.RoundCount += 1;
    ...
    /// </summary>
    public async Task<WFH_V3_Case> CancelAsync(Guid caseId, Guid actorUserId, CancellationToken ct)
    {
        var c = await LoadAsync(caseId, ct);
        if (c.SubmitterUserId != actorUserId)
            throw new ForbiddenException("only the original submitter may withdraw this case");
        if (c.Status is WFH_V3_CaseStatus.Completed or WFH_V3_CaseStatus.Cancelled)
        ...
        c.LastActivityAt = clock.UtcNow;
        await store.SaveChangesAsync(ct);
        log.LogInformation("WFH_V3/{CaseId}: withdrawn by submitter", c.Id);
        return c;
    }
    ...
    public Task<WFH_V3_Case> ApproveByManagerAsync(Guid caseId, Guid actorUserId, string? comment, CancellationToken ct)
        => ManagerDecisionAsync(caseId, actorUserId, approve: true, comment, ct);

    public Task<WFH_V3_Case> RejectByManagerAsync(Guid caseId, Guid actorUserId, string? comment, CancellationToken ct)
        => ManagerDecisionAsync(caseId, actorUserId, approve: false, comment, ct);

    private async Task<WFH_V3_Case> ManagerDecisionAsync(
        Guid caseId, Guid actorUserId, bool approve, string? comment, CancellationToken ct)
    {
        var c = await LoadAsync(caseId, ct);
        if (c.Status != WFH_V3_CaseStatus.PendingManager)
        ...
        // gateway_days: >= 30 consecutive days → senior approval, else complete.
        if (c.Days >= DaysGateThreshold)
        {
            ...
            c.Status = WFH_V3_CaseStatus.PendingSenior;
            await store.SaveChangesAsync(ct);
            log.LogInformation("WFH_V3/{CaseId}: manager approved, days {Days} >= {Threshold} → senior {Senior}",
        ...
        c.CompletedAt = clock.UtcNow;
        await store.SaveChangesAsync(ct);
        log.LogInformation("WFH_V3/{CaseId}: manager approved, days {Days} < {Threshold} → completed",
            c.Id, c.Days, DaysGateThreshold);
        return c;
    }

    // ------------------------------------------------------------
    // Senior stage (approval_senior) — only on the days >= 30 branch
    // ------------------------------------------------------------

    public Task<WFH_V3_Case> ApproveBySeniorAsync(Guid caseId, Guid actorUserId, string? comment, CancellationToken ct)
        => SeniorDecisionAsync(caseId, actorUserId, approve: true, comment, ct);

    public Task<WFH_V3_Case> RejectBySeniorAsync(Guid caseId, Guid actorUserId, string? comment, CancellationToken ct)
        => SeniorDecisionAsync(caseId, actorUserId, approve: false, comment, ct);

    private async Task<WFH_V3_Case> SeniorDecisionAsync(
        Guid caseId, Guid actorUserId, bool approve, string? comment, CancellationToken ct)
    {
        var c = await LoadAsync(caseId, ct);
        if (c.Status != WFH_V3_CaseStatus.PendingSenior)
        ...
        c.CompletedAt = clock.UtcNow;
        await store.SaveChangesAsync(ct);
        log.LogInformation("WFH_V3/{CaseId}: senior approved → completed", c.Id);
        return c;
    }
    ...
    /// </summary>
    private async Task<WFH_V3_Case> SendBackAsync(WFH_V3_Case c, string? comment, string defaultReason, CancellationToken ct)
    {
        c.Status = WFH_V3_CaseStatus.ResubmitRequired;
        c.CurrentAssigneeUserId = c.SubmitterUserId;
        await store.SaveChangesAsync(ct);
        log.LogInformation("WFH_V3/{CaseId}: rejected → resubmit required (round {Round})", c.Id, c.RoundCount);
        await NotifyRejectAsync(c, string.IsNullOrWhiteSpace(comment) ? defaultReason : comment!, ct);
        return c;
    }

    private async Task<WFH_V3_Case> LoadAsync(Guid caseId, CancellationToken ct)
        => await store.FindByIdAsync(caseId, ct)
           ?? throw new NotFoundException("WFH_V3_Case", caseId);
    ...
    private async Task NotifySubmittedAsync(WFH_V3_Case c, CancellationToken ct)
    {
        var submitter = await directory.GetByIdAsync(c.SubmitterUserId, ct);
        var rendered = WFH_V3_NotificationTemplates.RenderSubmitted($"/cases/wfh/{c.Id}");
        await notify.DispatchAsync(new NotifyMessage(
            SourceId:   "WFH_V3.notify_sgjz",
    ...
    private async Task NotifyAssignAsync(WFH_V3_Case c, Guid recipientUserId, CancellationToken ct)
    {
        ...
        var rendered = WFH_V3_NotificationTemplates.RenderAssign(
            applicantName, WFH_V3_NotificationTemplates.BuildSummary(c), $"/cases/wfh/{c.Id}");
        await notify.DispatchAsync(new NotifyMessage(
            SourceId:   "WFH_V3.notify_shnv",
            Subject:    rendered.Subject,
            Body:       rendered.Body,
            Channels:   new[] { "email", "in_app" },
    ...
    private async Task NotifyRejectAsync(WFH_V3_Case c, string rejectReason, CancellationToken ct)
    {
        var submitter = await directory.GetByIdAsync(c.SubmitterUserId, ct);
        var rendered = WFH_V3_NotificationTemplates.RenderReject(rejectReason, $"/cases/wfh/{c.Id}");
        await notify.DispatchAsync(new NotifyMessage(
            SourceId:   "WFH_V3.notify_slfp",
            Subject:    rendered.Subject,
            Body:       rendered.Body,
            Channels:   new[] { "email", "in_app" },
            Recipients: new[] { new NotifyRecipient(c.SubmitterUserId, submitter?.Email, submitter?.DisplayName) },
    ...
    private static IReadOnlyDictionary<string, string?> NotificationContext(WFH_V3_Case c)
