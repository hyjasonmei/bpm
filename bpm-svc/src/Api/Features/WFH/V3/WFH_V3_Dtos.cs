...
public sealed record WFH_V3_DateRange(DateOnly Start, DateOnly End);

public sealed record WFH_V3_SubmitRequest(
    DateOnly ApplyDate,
    WFH_V3_DateRange DateRange,
    string Reason,
    Guid? AttachmentFileId);

public sealed record WFH_V3_DecisionRequest(bool Approve, string? Comment);

public sealed record WFH_V3_DecisionDto(
    Guid? UserId, string? DisplayName, bool? Approved, string? Comment, DateTime? DecidedAt);

public sealed record WFH_V3_CaseResponse(
    Guid Id,
    ...
    WFH_V3_DecisionDto? ManagerDecision,
    WFH_V3_DecisionDto? SeniorDecision,
    DateTime SubmittedAt,
    DateTime LastActivityAt,
    DateTime? CompletedAt);

public sealed record WFH_V3_CaseRowResponse(
    Guid Id,
    int Days,
    DateOnly StartDate,
    DateOnly EndDate,
...
internal static class WFH_V3_DtoMapping
{
    public static WFH_V3_CaseResponse ToResponse(WFH_V3_Case c, IReadOnlyDictionary<Guid, string> names)
        => new(
            Id: c.Id,
            ...
            ManagerDecision: c.ManagerUserId is null ? null : new WFH_V3_DecisionDto(
                c.ManagerUserId, names.GetValueOrDefault(c.ManagerUserId.Value),
                c.ManagerApproved, c.ManagerComment, c.ManagerDecisionAt),
            SeniorDecision: c.SeniorUserId is null ? null : new WFH_V3_DecisionDto(
    ...
    public static WFH_V3_WfhService.SubmitInput ToServiceInput(Guid submitterUserId, WFH_V3_SubmitRequest req)
