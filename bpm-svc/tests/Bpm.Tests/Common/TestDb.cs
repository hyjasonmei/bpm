...
internal sealed class TestDb : IDisposable, IAsyncDisposable
{
    ...
    private TestDb(SqliteConnection conn, AppDbContext ctx) { _conn = conn; Context = ctx; }
    ...
    public async ValueTask DisposeAsync() { await Context.DisposeAsync(); await _conn.DisposeAsync(); }
}
