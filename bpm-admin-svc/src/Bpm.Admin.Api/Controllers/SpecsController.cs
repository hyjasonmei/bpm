...
using Bpm.Admin.Application.Spec.Expressions;
using Microsoft.AspNetCore.Mvc;

namespace Bpm.Admin.Api.Controllers;

/// <summary>
/// Live expression-validation endpoint for the AI Kitchen wizard. Ported
/// from bpm-svc/Spec/SpecValidationController so admin-svc owns the spec
/// authoring path end-to-end (bpm-svc still owns runtime evaluation).
///
/// Front-end debounces 500 ms and posts the in-progress expression so the
/// editor can show inline ✓ / ✗ status. Returns 200 even on validation
/// failures — the body's <c>valid</c> flag is the success signal, not
/// the HTTP status.
/// </summary>
[ApiController]
[Route("api/specs")]
public sealed class SpecsController(IExpressionEvaluator evaluator) : ControllerBase
{
