...
import { Settings, Users, Sparkles, Shield, Activity, ExternalLink, LogOut, Eye, Library, Mail, Workflow } from 'lucide-react'
import { cn } from '@/lib/cn'
import { decodeJwt } from '@/lib/jwt'
import { clearJwt, getJwt } from '@/lib/apiFetch'
import { SandboxBanner } from '@/components/SandboxBanner'
import { useSandboxUnreadCount } from '@/hooks/useSandboxUnreadCount'
...
const NAV: Array<{ kind: AdminScreen['kind']; label: string; icon: React.ReactNode }> = [
  ...
  { kind: 'processes',       label: 'Process Admin',   icon: <Workflow className="h-4 w-4" /> },
  ...
  { kind: 'sandbox-mailbox', label: 'Sandbox Mailbox', icon: <Mail     className="h-4 w-4" /> },
...
export function AdminLayout({ screen, setScreen, children, topBanner }: AdminLayoutProps) {
  ...
  const { count: unread, sandboxOn } = useSandboxUnreadCount()
      ...
      <div className="flex">
        <aside className="w-56 shrink-0 border-r border-rule bg-card">
          <nav className="sticky top-12 flex flex-col p-2">
            {NAV.map(item => {
              const showBadge = item.kind === 'sandbox-mailbox' && sandboxOn && unread.total > 0
              return (
                <button
                  key={item.kind}
                ...
                >
                  ...
                  {showBadge && (
                    <span
                      title={`${unread.total} unread captured message(s)`}
                      className="inline-flex h-4 min-w-[16px] items-center justify-center rounded-full bg-rose-500 px-1 text-[9.5px] font-bold leading-none text-white"
                    >
