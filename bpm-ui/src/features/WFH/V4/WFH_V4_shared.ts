import type { WFH_V4_Status, WFH_V4_SubmitPayload } from './WFH_V4_types'

export function emptyPayload(): WFH_V4_SubmitPayload {
  return { applyDate: '', start: '', end: '', reason: '', attachmentFileId: null }
}

/**
 * Senior approval (上級主管) kicks in at this many consecutive days
 * **or more** (spec gateway_days: `days >= 60`). V4 raised V3's >= 30 gate.
 */
export const DAYS_GATE_THRESHOLD = 60

/** Whether a request needs the senior (上級主管) stage — mirrors the C# gate `c.Days >= 60`. */
 ...
 * `WFH_V4_WfhService.ComputeConsecutiveDays`; the server recomputes and
...
export function zhStatus(s: WFH_V4_Status): string {
  switch (s) {
