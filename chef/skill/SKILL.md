...
1. **Write only inside the per-version feature folders inside the
   ...
   new csproj files or edit `bpm-svc.slnx`. Allowed write paths:
   - `bpm-svc/src/Persistence/Features/<CODE>/V<N>/**` — entity, EF
     configuration, service / state machine, notification templates,
     inbox provider
...
the LEAVE V1 reference set on `leave-test-4`:

```
bpm-svc/src/Persistence/Features/LEAVE/V1/
├── LEAVE_V1_Case.cs                     ← EF entity: business data
...
├── LEAVE_V1_CaseConfiguration.cs        ← EF mapping
...
`LEAVE_V1_InboxProvider` plugs into the backend the same way:
`Persistence.DependencyInjection` scans this assembly for
non-abstract `ITypedInboxProvider` types at startup and registers
each. Chef just drops the file and ships.
...
7. The LEAVE V1 reference set on `leave-test-4` if you've never cooked
   in model B before — copy the shape, not the code.
...
`EnsureCreated()` builds chef-owned tables from the model. SharedIdentity
configurations carry `ExcludeFromMigrations` (admin-svc owns those
tables in production), so test setup must CREATE TABLE the
`Admin_Principals` / `Admin_UserManagers` / `Admin_UserDepts` /
`Admin_DeptHeads` / `Admin_Roles` / `Admin_PrincipalRoles` subset by
raw SQL. The LEAVE_V1 test file has the canonical block — copy it.
