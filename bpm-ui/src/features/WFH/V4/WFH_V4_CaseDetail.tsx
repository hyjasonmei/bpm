...
import { zhStatus } from './WFH_V4_shared'
import type { WFH_V4_CaseResponse, WFH_V4_Status } from './WFH_V4_types'

/**
 * Read-mostly detail page for a WFH V4 case. Role-scoped ActionFooter:
 ...
 */
export function WFH_V4_CaseDetail({ caseId }: CaseDetailProps) {
  const navigate = useNavigate()
  const [data, setData] = useState<WFH_V4_CaseResponse | null>(null)
  ...
  const load = useCallback(async () => {
    try {
      const res = await apiFetch(`/api/wfh/v4/${caseId}`)
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      setData((await res.json()) as WFH_V4_CaseResponse)
      setError(null)
    } catch (e) {
  ...
  const postDecision = useCallback(async (approve: boolean) => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/wfh/v4/${caseId}/${path}`, {
        method: 'POST',
  ...
  const postCancel = useCallback(async () => {
    setActionPending(true); setActionError(null)
    try {
      const res = await apiFetch(`/api/wfh/v4/${caseId}/cancel`, { method: 'POST' })
      if (!res.ok) throw new Error((await res.text()) || `HTTP ${res.status}`)
      await load()
    } catch (e) {
      setActionError(e instanceof Error ? e.message : String(e))
  ...
  return (
    <div className="mx-auto max-w-screen-lg space-y-4 p-6 pb-24">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <h1 className="truncate text-lg font-bold text-ink">
            居家辦公案件 <span className="ml-2 text-base font-medium text-ink-muted">· WFH V4</span>
          </h1>
      ...
      {data && (
        <>
          <FlowStateBanner flowCode="WFH" flowVersion={4} />
...
function statusKind(s: WFH_V4_Status): StatusKind {
  switch (s) {
...
function activeStepFor(status: WFH_V4_Status): number {
  switch (status) {
 ...
 */
function deriveTrail(c: WFH_V4_CaseResponse): { completed: string[]; current: string | null } {
