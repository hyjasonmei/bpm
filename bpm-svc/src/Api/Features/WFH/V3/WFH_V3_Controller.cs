...
namespace Bpm.Api.Features.WFH.V3;

[ApiController]
[Authorize]
[Route("api/wfh/v3")]
public sealed class WFH_V3_Controller(
    WFH_V3_WfhService service,
    IWFH_V3_CaseStore store,
    IPrincipalDirectory directory) : BpmControllerBase
{
    [HttpPost]
    public async Task<ActionResult<WFH_V3_CaseResponse>> Submit([FromBody] WFH_V3_SubmitRequest req, CancellationToken ct)
    {
        var userId = RequireUserId();
        var c = await service.SubmitAsync(WFH_V3_DtoMapping.ToServiceInput(userId, req), ct);
        return CreatedAtAction(nameof(GetById), new { caseId = c.Id }, await BuildResponseAsync(c, ct));
    }

    [HttpPost("{caseId:guid}/resubmit")]
    public async Task<ActionResult<WFH_V3_CaseResponse>> Resubmit(Guid caseId, [FromBody] WFH_V3_SubmitRequest req, CancellationToken ct)
    {
        var userId = RequireUserId();
        var c = await service.ResubmitAsync(caseId, userId, WFH_V3_DtoMapping.ToServiceInput(userId, req), ct);
        return await BuildResponseAsync(c, ct);
    }

    [HttpPost("{caseId:guid}/manager-decision")]
    public async Task<ActionResult<WFH_V3_CaseResponse>> ManagerDecision(Guid caseId, [FromBody] WFH_V3_DecisionRequest req, CancellationToken ct)
    {
        var userId = RequireUserId();
        var c = req.Approve
    ...
    public async Task<ActionResult<WFH_V3_CaseResponse>> SeniorDecision(Guid caseId, [FromBody] WFH_V3_DecisionRequest req, CancellationToken ct)
    {
        var userId = RequireUserId();
        var c = req.Approve
    ...
    public async Task<ActionResult<WFH_V3_CaseResponse>> Cancel(Guid caseId, CancellationToken ct)
    {
        var userId = RequireUserId();
    ...
    [HttpGet("{caseId:guid}")]
    public async Task<ActionResult<WFH_V3_CaseResponse>> GetById(Guid caseId, CancellationToken ct)
    {
    ...
    [HttpGet("mine")]
    public async Task<IReadOnlyList<WFH_V3_CaseRowResponse>> Mine(CancellationToken ct)
        => await BuildRowsAsync(await store.FindMineAsync(RequireUserId(), ct), ct);

    [HttpGet("pending")]
    public async Task<IReadOnlyList<WFH_V3_CaseRowResponse>> Pending(CancellationToken ct)
        => await BuildRowsAsync(await store.FindPendingAsync(RequireUserId(), ct), ct);

    private async Task<WFH_V3_CaseResponse> BuildResponseAsync(WFH_V3_Case c, CancellationToken ct)
    {
        ...
        return WFH_V3_DtoMapping.ToResponse(c, lookups.ToDictionary(kv => kv.Key, kv => kv.Value.DisplayName));
    }

    private async Task<IReadOnlyList<WFH_V3_CaseRowResponse>> BuildRowsAsync(IReadOnlyList<WFH_V3_Case> cases, CancellationToken ct)
    {
        if (cases.Count == 0) return Array.Empty<WFH_V3_CaseRowResponse>();
        ...
        return cases.Select(c => new WFH_V3_CaseRowResponse(
