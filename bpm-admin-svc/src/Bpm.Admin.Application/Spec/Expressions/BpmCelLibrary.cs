...
using NodaTime;
using IClock = Bpm.Admin.Application.Common.Abstractions.IClock;

namespace Bpm.Admin.Application.Spec.Expressions;

/// <summary>
/// bpm-cel-v1 custom functions registered as a CEL library. Ported from
/// bpm-svc/Application/Spec/Expressions/BpmCelLibrary.cs so the wizard's
/// `validate-expression` endpoint can live on admin-svc (AI / authoring
/// IP stays here; bpm-svc still owns runtime).
///
...
public sealed class BpmCelLibrary(IClock clock) : ILibrary
{
    ...
    public IList<ProgramOption> ProgramOptions => new List<ProgramOption>
    {
        global::Cel.ProgramOptions.Functions(
            ...
            {
                // v1 stub: returns natural days. Mirrors bpm-svc behaviour
                // until add-calendar-and-business-hours ships there.
                var t1 = ExtractInstant(args[0]);
                var t2 = ExtractInstant(args[1]);
                var span = (long)(t2 - t1).TotalDays;
                return IntT.IntOf(span);
