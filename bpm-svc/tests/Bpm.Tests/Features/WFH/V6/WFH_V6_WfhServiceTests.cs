...
namespace Bpm.Tests.Features.WFH.V6;

/// <summary>
/// Unit tests for the WFH V6 state machine. Org chain: Emily → Mike →
/// Vera (top). days &lt; 100 completes at manager; days &gt;= 100 routes to
/// senior = submitter.manager.manager. (V6 raises V5's &gt;= 90 gateway to
/// &gt;= 100.) Real EF-backed store + directory + org reader against
/// in-memory SQLite.
/// </summary>
public sealed class WFH_V6_WfhServiceTests : IDisposable
{
    ...
    public async Task Submit_happy_path_routes_to_manager()
    {
        ...
        Assert.Equal(WFH_V6_CaseStatus.PendingManager, c.Status);
    ...
    public async Task Submit_long_request_computes_days_over_threshold()
    {
        ...
        Assert.Equal(120, c.Days); // 7/1..10/28 inclusive
    }
    ...
    public async Task Manager_approve_short_completes_case()
    {
        ...
        Assert.Equal(WFH_V6_CaseStatus.Completed, done.Status);
    ...
    {
        // Exactly 100 days must route to senior (gateway is days >= 100).
        await using var db = new AppDbContext(_options);
        var svc = NewService(db);
        ...
        Assert.Equal(WFH_V6_CaseStatus.PendingSenior, after.Status);
    ...
    {
        // 99 days (one below the gate) must complete at manager.
        await using var db = new AppDbContext(_options);
        var svc = NewService(db);
        var c = await svc.SubmitAsync(ShortRequest() with { EndDate = new DateOnly(2026, 10, 7) }, default);
        Assert.Equal(99, c.Days);
        var done = await svc.ApproveByManagerAsync(c.Id, Mike, "ok", default);
        Assert.Equal(WFH_V6_CaseStatus.Completed, done.Status);
        Assert.Null(done.SeniorUserId);
    }
    ...
    public async Task Manager_reject_returns_to_submitter()
    {
        ...
        Assert.Equal(WFH_V6_CaseStatus.ResubmitRequired, after.Status);
    ...
    {
        // Submitter = Mike (manager = Vera, Vera has no manager). days >= 100 →
    ...
    public async Task Resubmit_after_reject_starts_round_2()
    {
        ...
        Assert.Equal(WFH_V6_CaseStatus.PendingManager, after.Status);
    ...
    public async Task Submitter_withdraws_pending_case()
    {
        ...
        Assert.Equal(WFH_V6_CaseStatus.Cancelled, after.Status);
    ...
    {
        Assert.Equal(1, WFH_V6_WfhService.ComputeConsecutiveDays(new DateOnly(2026, 7, 1), new DateOnly(2026, 7, 1)));
        Assert.Equal(30, WFH_V6_WfhService.ComputeConsecutiveDays(new DateOnly(2026, 7, 1), new DateOnly(2026, 7, 30)));
        Assert.Equal(0, WFH_V6_WfhService.ComputeConsecutiveDays(new DateOnly(2026, 7, 8), new DateOnly(2026, 7, 1)));
    }

    [Fact]
    public void NotificationTemplate_Assign_renders_summary()
    {
        var r = WFH_V6_NotificationTemplates.RenderAssign("Emily", "居家辦公 — 100 天（2026/07/01 ~ 2026/10/08）", "/cases/wfh/x");
    ...
    {
        var r = WFH_V6_NotificationTemplates.RenderReject("天數過長", "/cases/wfh/x");
    ...
    public async Task E2E_long_request_manager_then_senior_completes()
    {
        ...
        Assert.Equal(WFH_V6_CaseStatus.PendingSenior, afterMgr.Status);
        var done = await svc.ApproveBySeniorAsync(c.Id, Vera, "approved", default);
        Assert.Equal(WFH_V6_CaseStatus.Completed, done.Status);
        Assert.True(done.ManagerApproved);
        Assert.True(done.SeniorApproved);
    ...
    private static WFH_V6_WfhService NewService(AppDbContext db, INotifyDispatcher? notify = null)
        => new(new WFH_V6_CaseStore(db), new OrgChartReader(db), new PrincipalDirectory(db),
               new StubClock(), NullLogger<WFH_V6_WfhService>.Instance, notify ?? new NullNotifyDispatcher(), new TestActorAuthorizer());

    // Short = 5 days (< 100) → completes at manager.
    private static WFH_V6_WfhService.SubmitInput ShortRequest()
        => new(Emily, new DateOnly(2026, 6, 20), new DateOnly(2026, 7, 1), new DateOnly(2026, 7, 5), "家中有事需居家辦公", null);

    // Threshold = exactly 100 days → routes to senior.
    private static WFH_V6_WfhService.SubmitInput ThresholdRequest()
        => new(Emily, new DateOnly(2026, 6, 20), new DateOnly(2026, 7, 1), new DateOnly(2026, 10, 8), "需連續超過三個月居家辦公", null);

    // Long = 120 days (>= 100) → routes to senior.
    private static WFH_V6_WfhService.SubmitInput LongRequest()
        => new(Emily, new DateOnly(2026, 6, 20), new DateOnly(2026, 7, 1), new DateOnly(2026, 10, 28), "長期居家辦公需求", null);
