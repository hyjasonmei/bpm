...
import { zhStatus } from './WFH_V3_shared'
import type { WFH_V3_CaseResponse, WFH_V3_Status } from './WFH_V3_types'

/**
 * Read-mostly detail page for a WFH V3 case. Role-scoped ActionFooter:
 ...
 */
export function WFH_V3_CaseDetail({ caseId }: CaseDetailProps) {
  const navigate = useNavigate()
  const [data, setData] = useState<WFH_V3_CaseResponse | null>(null)
  ...
  const load = useCallback(async () => {
    try {
      const res = await apiFetch(`/api/wfh/v3/${caseId}`)
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      setData((await res.json()) as WFH_V3_CaseResponse)
      setError(null)
    } catch (e) {
  ...
  const postDecision = useCallback(async (approve: boolean) => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/wfh/v3/${caseId}/${path}`, {
        method: 'POST',
  ...
  const postCancel = useCallback(async () => {
    setActionPending(true); setActionError(null)
    try {
      const res = await apiFetch(`/api/wfh/v3/${caseId}/cancel`, { method: 'POST' })
      if (!res.ok) throw new Error((await res.text()) || `HTTP ${res.status}`)
      await load()
    } catch (e) {
      setActionError(e instanceof Error ? e.message : String(e))
  ...
  return (
    <div className="mx-auto max-w-screen-lg space-y-4 p-6 pb-24">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="order-last w-full min-w-0 md:order-none md:w-auto md:flex-1">
          <h1 className="truncate text-lg font-bold text-ink">
            居家辦公案件 <span className="ml-2 text-base font-medium text-ink-muted">· WFH V3</span>
          </h1>
      ...
      {data && (
        <>
          <FlowStateBanner flowCode="WFH" flowVersion={3} />
...
function statusKind(s: WFH_V3_Status): StatusKind {
  switch (s) {
...
function activeStepFor(status: WFH_V3_Status): number {
  switch (status) {
 ...
 */
function deriveTrail(c: WFH_V3_CaseResponse): { completed: string[]; current: string | null } {
