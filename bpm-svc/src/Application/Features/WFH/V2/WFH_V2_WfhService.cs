...
/// <summary>
/// State machine for WFH (居家辦公申請) V2. One transition per public
/// method. V2 raises the senior-approval gateway threshold from V1's
/// &gt; 7 days to <b>≥ 15 days</b> (bundle edges e4 <c>days &lt; 15</c> →
/// end_approved, e5 <c>days &gt;= 15</c> → approval_senior). The graph is
/// otherwise identical to V1. Status graph (bundle nodes start_1 /
/// task_apply / approval_manager / gateway_days / approval_senior /
/// end_*):
/// <code>
///   start ─► PendingManager ──approve, days &lt; 15──► Completed
///                  │                  │
///                  │              approve, days &gt;= 15
///                  │                  ▼
...
/// <c>submitter.manager.manager</c> (spec writes this explicitly in V2,
/// with fallback "若往上層找已經沒有主管，就用該主管" → falls back to the
/// manager itself when the manager is already top of the org chain).
/// </summary>
public sealed class WFH_V2_WfhService(
    IWFH_V2_CaseStore store,
    IOrgChartReader org,
    IPrincipalDirectory directory,
    IClock clock,
    ILogger<WFH_V2_WfhService> log,
    INotifyDispatcher notify,
    IActorAuthorizer auth)
{
    public const string FlowCode = "WFH";
    public const int FlowVersion = 2;

    /// <summary>gateway_days threshold — consecutive days &gt;= 15 routes to senior approval.</summary>
    public const int DaysGateThreshold = 15;
    ...
    public async Task<WFH_V2_Case> SubmitAsync(SubmitInput input, CancellationToken ct)
    {
        ...
        var c = new WFH_V2_Case
        {
            ...
            Status = WFH_V2_CaseStatus.PendingManager,
        ...
        Guid caseId, Guid actorUserId, SubmitInput input, CancellationToken ct)
    {
        ...
        c.Status = WFH_V2_CaseStatus.PendingManager;
        c.RoundCount += 1;
    ...
    /// </summary>
    public async Task<WFH_V2_Case> CancelAsync(Guid caseId, Guid actorUserId, CancellationToken ct)
    {
        var c = await LoadAsync(caseId, ct);
        if (c.SubmitterUserId != actorUserId)
            throw new ForbiddenException("only the original submitter may withdraw this case");
        if (c.Status is WFH_V2_CaseStatus.Completed or WFH_V2_CaseStatus.Cancelled)
        ...
        c.LastActivityAt = clock.UtcNow;
        await store.SaveChangesAsync(ct);
        log.LogInformation("WFH_V2/{CaseId}: withdrawn by submitter", c.Id);
        return c;
    }
    ...
    public Task<WFH_V2_Case> ApproveByManagerAsync(Guid caseId, Guid actorUserId, string? comment, CancellationToken ct)
        => ManagerDecisionAsync(caseId, actorUserId, approve: true, comment, ct);

    public Task<WFH_V2_Case> RejectByManagerAsync(Guid caseId, Guid actorUserId, string? comment, CancellationToken ct)
        => ManagerDecisionAsync(caseId, actorUserId, approve: false, comment, ct);

    private async Task<WFH_V2_Case> ManagerDecisionAsync(
        Guid caseId, Guid actorUserId, bool approve, string? comment, CancellationToken ct)
    {
        var c = await LoadAsync(caseId, ct);
        if (c.Status != WFH_V2_CaseStatus.PendingManager)
        ...
        // gateway_days: >= 15 consecutive days → senior approval, else complete.
        if (c.Days >= DaysGateThreshold)
        {
            ...
            c.Status = WFH_V2_CaseStatus.PendingSenior;
            await store.SaveChangesAsync(ct);
            log.LogInformation("WFH_V2/{CaseId}: manager approved, days {Days} >= {Threshold} → senior {Senior}",
        ...
        c.CompletedAt = clock.UtcNow;
        await store.SaveChangesAsync(ct);
        log.LogInformation("WFH_V2/{CaseId}: manager approved, days {Days} < {Threshold} → completed",
            c.Id, c.Days, DaysGateThreshold);
        return c;
    }

    // ------------------------------------------------------------
    // Senior stage (approval_senior) — only on the days >= 15 branch
    // ------------------------------------------------------------

    public Task<WFH_V2_Case> ApproveBySeniorAsync(Guid caseId, Guid actorUserId, string? comment, CancellationToken ct)
        => SeniorDecisionAsync(caseId, actorUserId, approve: true, comment, ct);

    public Task<WFH_V2_Case> RejectBySeniorAsync(Guid caseId, Guid actorUserId, string? comment, CancellationToken ct)
        => SeniorDecisionAsync(caseId, actorUserId, approve: false, comment, ct);

    private async Task<WFH_V2_Case> SeniorDecisionAsync(
        Guid caseId, Guid actorUserId, bool approve, string? comment, CancellationToken ct)
    {
        var c = await LoadAsync(caseId, ct);
        if (c.Status != WFH_V2_CaseStatus.PendingSenior)
        ...
        c.CompletedAt = clock.UtcNow;
        await store.SaveChangesAsync(ct);
        log.LogInformation("WFH_V2/{CaseId}: senior approved → completed", c.Id);
        return c;
    }
    ...
    /// </summary>
    private async Task<WFH_V2_Case> SendBackAsync(WFH_V2_Case c, string? comment, string defaultReason, CancellationToken ct)
    {
        c.Status = WFH_V2_CaseStatus.ResubmitRequired;
        c.CurrentAssigneeUserId = c.SubmitterUserId;
        await store.SaveChangesAsync(ct);
        log.LogInformation("WFH_V2/{CaseId}: rejected → resubmit required (round {Round})", c.Id, c.RoundCount);
        await NotifyRejectAsync(c, string.IsNullOrWhiteSpace(comment) ? defaultReason : comment!, ct);
        return c;
    }

    private async Task<WFH_V2_Case> LoadAsync(Guid caseId, CancellationToken ct)
        => await store.FindByIdAsync(caseId, ct)
           ?? throw new NotFoundException("WFH_V2_Case", caseId);
    ...
    /// <summary>
    /// Senior-stage approver. V2 spec writes <c>submitter.manager.manager</c>
    ...
    private async Task NotifySubmittedAsync(WFH_V2_Case c, CancellationToken ct)
    {
        var submitter = await directory.GetByIdAsync(c.SubmitterUserId, ct);
        var rendered = WFH_V2_NotificationTemplates.RenderSubmitted($"/cases/wfh/{c.Id}");
        await notify.DispatchAsync(new NotifyMessage(
            SourceId:   "WFH_V2.notify_sgjz",
    ...
    private async Task NotifyAssignAsync(WFH_V2_Case c, Guid recipientUserId, CancellationToken ct)
    {
        ...
        var rendered = WFH_V2_NotificationTemplates.RenderAssign(
            applicantName, WFH_V2_NotificationTemplates.BuildSummary(c), $"/cases/wfh/{c.Id}");
        await notify.DispatchAsync(new NotifyMessage(
            SourceId:   "WFH_V2.notify_shnv",
            Subject:    rendered.Subject,
            Body:       rendered.Body,
            Channels:   new[] { "email", "in_app" },
    ...
    private async Task NotifyRejectAsync(WFH_V2_Case c, string rejectReason, CancellationToken ct)
    {
        var submitter = await directory.GetByIdAsync(c.SubmitterUserId, ct);
        var rendered = WFH_V2_NotificationTemplates.RenderReject(rejectReason, $"/cases/wfh/{c.Id}");
        await notify.DispatchAsync(new NotifyMessage(
            SourceId:   "WFH_V2.notify_slfp",
            Subject:    rendered.Subject,
            Body:       rendered.Body,
            Channels:   new[] { "email", "in_app" },
            Recipients: new[] { new NotifyRecipient(c.SubmitterUserId, submitter?.Email, submitter?.DisplayName) },
    ...
    private static IReadOnlyDictionary<string, string?> NotificationContext(WFH_V2_Case c)
