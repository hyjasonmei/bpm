...
namespace Bpm.Tests.Features.WFH.V4;

/// <summary>
/// Unit tests for the WFH V4 state machine. Org chain: Emily → Mike →
/// Vera (top). days &lt; 60 completes at manager; days &gt;= 60 routes to
/// senior = submitter.manager.manager. (V4 raises V2's &gt;= 15 gateway to
/// &gt;= 60.) Real EF-backed store + directory + org reader against
/// in-memory SQLite.
/// </summary>
public sealed class WFH_V4_WfhServiceTests : IDisposable
{
    ...
    public async Task Submit_happy_path_routes_to_manager()
    {
        ...
        Assert.Equal(WFH_V4_CaseStatus.PendingManager, c.Status);
    ...
    public async Task Submit_long_request_computes_days_over_threshold()
    {
        ...
        Assert.Equal(70, c.Days); // 7/1..9/8 inclusive
    }
    ...
    public async Task Manager_approve_short_completes_case()
    {
        ...
        Assert.Equal(WFH_V4_CaseStatus.Completed, done.Status);
    ...
    {
        // Exactly 60 days must route to senior (gateway is days >= 60).
        await using var db = new AppDbContext(_options);
        var svc = NewService(db);
        var c = await svc.SubmitAsync(ThresholdRequest(), default);
        Assert.Equal(60, c.Days); // 7/1..8/29 inclusive
        var after = await svc.ApproveByManagerAsync(c.Id, Mike, "ok", default);
        Assert.Equal(WFH_V4_CaseStatus.PendingSenior, after.Status);
    ...
    {
        // 59 days (one below the gate) must complete at manager.
        await using var db = new AppDbContext(_options);
        var svc = NewService(db);
        var c = await svc.SubmitAsync(ShortRequest() with { EndDate = new DateOnly(2026, 8, 28) }, default);
        Assert.Equal(59, c.Days);
        var done = await svc.ApproveByManagerAsync(c.Id, Mike, "ok", default);
        Assert.Equal(WFH_V4_CaseStatus.Completed, done.Status);
        Assert.Null(done.SeniorUserId);
    }
    ...
    public async Task Manager_reject_returns_to_submitter()
    {
        ...
        Assert.Equal(WFH_V4_CaseStatus.ResubmitRequired, after.Status);
    ...
    {
        // Submitter = Mike (manager = Vera, Vera has no manager). days >= 60 →
    ...
    public async Task Resubmit_after_reject_starts_round_2()
    {
        ...
        Assert.Equal(WFH_V4_CaseStatus.PendingManager, after.Status);
    ...
    public async Task Submitter_withdraws_pending_case()
    {
        ...
        Assert.Equal(WFH_V4_CaseStatus.Cancelled, after.Status);
    ...
    {
        Assert.Equal(1, WFH_V4_WfhService.ComputeConsecutiveDays(new DateOnly(2026, 7, 1), new DateOnly(2026, 7, 1)));
        Assert.Equal(30, WFH_V4_WfhService.ComputeConsecutiveDays(new DateOnly(2026, 7, 1), new DateOnly(2026, 7, 30)));
        Assert.Equal(0, WFH_V4_WfhService.ComputeConsecutiveDays(new DateOnly(2026, 7, 8), new DateOnly(2026, 7, 1)));
    }

    [Fact]
    public void NotificationTemplate_Assign_renders_summary()
    {
        var r = WFH_V4_NotificationTemplates.RenderAssign("Emily", "居家辦公 — 60 天（2026/07/01 ~ 2026/08/29）", "/cases/wfh/x");
    ...
    {
        var r = WFH_V4_NotificationTemplates.RenderReject("天數過長", "/cases/wfh/x");
    ...
    public async Task E2E_long_request_manager_then_senior_completes()
    {
        ...
        Assert.Equal(WFH_V4_CaseStatus.PendingSenior, afterMgr.Status);
        var done = await svc.ApproveBySeniorAsync(c.Id, Vera, "approved", default);
        Assert.Equal(WFH_V4_CaseStatus.Completed, done.Status);
        Assert.True(done.ManagerApproved);
        Assert.True(done.SeniorApproved);
    ...
    private static WFH_V4_WfhService NewService(AppDbContext db, INotifyDispatcher? notify = null)
        => new(new WFH_V4_CaseStore(db), new OrgChartReader(db), new PrincipalDirectory(db),
               new StubClock(), NullLogger<WFH_V4_WfhService>.Instance, notify ?? new NullNotifyDispatcher(), new TestActorAuthorizer());

    // Short = 5 days (< 60) → completes at manager.
    private static WFH_V4_WfhService.SubmitInput ShortRequest()
        => new(Emily, new DateOnly(2026, 6, 20), new DateOnly(2026, 7, 1), new DateOnly(2026, 7, 5), "家中有事需居家辦公", null);

    // Threshold = exactly 60 days → routes to senior.
    private static WFH_V4_WfhService.SubmitInput ThresholdRequest()
        => new(Emily, new DateOnly(2026, 6, 20), new DateOnly(2026, 7, 1), new DateOnly(2026, 8, 29), "需連續兩個月居家辦公", null);

    // Long = 70 days (>= 60) → routes to senior.
    private static WFH_V4_WfhService.SubmitInput LongRequest()
        => new(Emily, new DateOnly(2026, 6, 20), new DateOnly(2026, 7, 1), new DateOnly(2026, 9, 8), "長期居家辦公需求", null);
