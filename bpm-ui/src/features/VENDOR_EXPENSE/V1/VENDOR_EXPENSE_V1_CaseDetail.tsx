...
export function VENDOR_EXPENSE_V1_CaseDetail({ caseId }: CaseDetailProps) {
  ...
  // The procurement step is a shared role queue (role code, no assignee user
  // id), so the userId equality check below can never match there — the
  // /pending set is the server truth for role-queue steps (inheritance-aware).
  const [canActOnCase, setCanActOnCase] = useState(false)

  const load = useCallback(async () => {
    try {
      const [res, pendingRes] = await Promise.all([
        apiFetch(`/api/vendor-expense/v1/${caseId}`),
        apiFetch(`/api/vendor-expense/v1/pending`),
      ])
  ...
  // pending set — which is what grants the role-queue procurement step.
