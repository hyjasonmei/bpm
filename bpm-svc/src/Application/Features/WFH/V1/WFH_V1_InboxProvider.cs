...
/// <summary>
/// Surface WFH V1 cases on the unified inbox. "Mine" = cases the user
/// submitted; "Pending" = the manager / senior approval step or a
/// rejected case bounced back to the submitter for resubmit.
/// </summary>
public sealed class WFH_V1_InboxProvider(
    IWFH_V1_CaseStore store,
    IPrincipalDirectory directory) : ITypedInboxProvider
{
    public string FlowCode => WFH_V1_WfhService.FlowCode;
    public int FlowVersion => WFH_V1_WfhService.FlowVersion;
    ...
    private static string ZhStatus(WFH_V1_CaseStatus s) => s switch
    {
        WFH_V1_CaseStatus.PendingManager   => "待主管核准",
        WFH_V1_CaseStatus.PendingSenior    => "待上級主管核准",
        WFH_V1_CaseStatus.ResubmitRequired => "退回補件",
        WFH_V1_CaseStatus.Completed        => "已核准",
        WFH_V1_CaseStatus.Cancelled        => "已撤回",
        _ => s.ToString(),
    };
}
